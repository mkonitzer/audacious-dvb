...
dvb_about (void)
{
  ...
  gtk_show_about_dialog(NULL,
			...
			"version", VERSION,
			"copyright", "Copyright (C) 2007  Marius Konitzer\n\n"
			"Based on xmms-dvb:\n"
			"Copyright (C) 2003, 2004  Christian Motz",
			"authors", authors,
			"license",
...
dvb_configure (void)
{
  ...
  GtkWidget *recordCheck = gtk_check_button_new_with_label("Record stream while playing");
  ...
  GtkWidget* appendCheck = gtk_check_button_new_with_label("Append to existing file");
  ...
  hbox = gtk_hbox_new(FALSE, 5);
  GtkWidget* isplitCheck = gtk_check_button_new_with_label("Split into intervals of:");
  ...
  hbox = gtk_hbox_new(FALSE, 5);
  GtkWidget* vsplitCheck = gtk_check_button_new_with_label("Split when volume is below:");
  ...
  GtkWidget* epgCheck = gtk_check_button_new_with_label("Electronic Program Guide (EPG)");
  widgets.epgCheck = epgCheck;
  gtk_box_pack_start(GTK_BOX(vbox), epgCheck, FALSE, FALSE, 0);
  
  GtkWidget* madCheck = gtk_check_button_new_with_label("MadMusic OpenTV Application");
...
{
  gboolean override;
  override = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.recordCheck));
  gtk_widget_set_sensitive(widgets.fnameLabel, override);
  gtk_widget_set_sensitive(widgets.fnameEntry, override);
  gtk_widget_set_sensitive(widgets.appendCheck, override);
  gtk_widget_set_sensitive(widgets.splitFrame, override);
}
...
{
  gboolean override;
  override = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.isplitCheck));
  gtk_widget_set_sensitive(widgets.isplitSpin, override);
}
...
{
  gboolean override;
  override = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.vsplitCheck));
  gtk_widget_set_sensitive(widgets.vsplitvolSpin, override);
  gtk_widget_set_sensitive(widgets.vsplit1Label, override);
  gtk_widget_set_sensitive(widgets.vsplit2Label, override);
  gtk_widget_set_sensitive(widgets.vsplitdurSpin, override);
  gtk_widget_set_sensitive(widgets.vsplit3Label, override);
  gtk_widget_set_sensitive(widgets.vsplit4Label, override);
  gtk_widget_set_sensitive(widgets.vsplitminlenSpin, override);
  gtk_widget_set_sensitive(widgets.vsplit5Label, override);
}
...
config_to_gui(cfgstruct *config)
{
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(widgets.loggingCombo), config->loglvl);
  
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.recordCheck), config->rec);
  gtk_entry_set_text(GTK_ENTRY(widgets.fnameEntry), config->rec_fname);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.appendCheck), config->rec_append);
  
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.isplitCheck), config->isplit);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(widgets.isplitSpin), config->isplit_ival);
  
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.vsplitCheck), config->vsplit);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(widgets.vsplitvolSpin), config->vsplit_vol);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(widgets.vsplitdurSpin), config->vsplit_dur);
  gtk_spin_button_set_value(GTK_SPIN_BUTTON(widgets.vsplitminlenSpin), config->vsplit_minlen);

  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.epgCheck), config->info_epg);
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widgets.madCheck), config->info_mmusic);
...
config_from_gui(cfgstruct *config)
{
  ...
  config->devpath = g_strdup(gtk_entry_get_text(GTK_ENTRY(widgets.devpathEntry)));
  config->loglvl = gtk_combo_box_get_active(GTK_COMBO_BOX(widgets.loggingCombo));
  
  config->rec = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.recordCheck));
  if (config->rec_fname != NULL)
      g_free(config->rec_fname);
  config->rec_fname = g_strdup(gtk_entry_get_text(GTK_ENTRY(widgets.fnameEntry)));
  config->rec_append = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.appendCheck));
  
  config->isplit = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.isplitCheck));
  config->isplit_ival = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widgets.isplitSpin));
  
  config->vsplit = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.vsplitCheck));
  config->vsplit_vol = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widgets.vsplitvolSpin));
  config->vsplit_dur = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widgets.vsplitdurSpin));
  config->vsplit_minlen = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widgets.vsplitminlenSpin));

  config->info_epg = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.epgCheck));
  config->info_mmusic = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(widgets.madCheck));
}
